...
// モックデータ
const introspectionData: ZodIntrospectionData[] = [
  {
    id: '1',
    date: '2025-03-22',
...
export const handlers = [
  ...
  http.post('/api/introspections', async ({ request }) => {
    const newEntry = (await request.json()) as ZodIntrospectionData
  ...
  http.put('/api/introspections/:id', async ({ request }) => {
    const updatedEntry = (await request.json()) as ZodIntrospectionData
  ...
  http.put('/api/goals/progress', async ({ request }) => {
    const updatedGoals = (await request.json()) as ZodGoalProgress[]
